# 🎬 Viron Button: The Eye Candy Blueprint

> [!NOTE]
> This is a high-end technical reference for the **Viron Button Ecosystem**. It combines cinematic 3D rendering (React Three Fiber) with AI-generated textures and advanced post-processing.
>
> **How to use this guide:**
>
> - ⚙️ **Tier 1-3:** Technical layers for R3F implementation.
> - 🧠 **AI Workflows:** ComfyUI & Luma integration for textures.
> - ⚠️ **Critical Sync:** Pay attention to the Dependency Matrix to avoid build failures.
...
## 🎨 Design Philosophy: The Viron Aesthetic

Before diving into code, understand the three pillars of the Viron look:

1. **Material Truth:** Glass must refract, metal must reflect. Use PBR (Physically Based Rendering) values.
2. **Generative Synergy:** Never use static textures. Combine AI-generated motion (Luma) with real-time refraction.
3. **Intentional Imperfection:** Use `Sparkles` and `Noise` to break the "perfect" digital look.

---

## 🍬 TIER 1: "Eye Candy" Stack (The Glass Core)
...
```json
{
  "nodes": {
    "1": {
      ...
      "inputs": { "ckpt_name": "flux_metal_jacket_v2.safetensors" }
    },
    "2": {
      "class_type": "SeamlessTile",
      "inputs": { "image": [1, 0], "tile_x": true, "tile_y": true }
    },
    "3": {
      "class_type": "KSampler",
      "inputs": {
        ...
        "cfg": 7.5,
        "model": [1, 0],
        "positive": "liquid chrome metal surface macro shot",
        "negative": "text watermark objects"
      }
    },
    "4": { "class_type": "VAEDecode", "inputs": { "samples": [3, 0] } },
    "5": {
      "class_type": "MarigoldDepthEstimator",
      "inputs": { "image": [4, 0] }
    },
    "6": {
      "class_type": "SaveImage",
      "inputs": { "images": [4, 0], "filename_prefix": "metal_diffuse" }
    },
    "7": {
      "class_type": "SaveImage",
      "inputs": { "images": [5, 0], "filename_prefix": "metal_normal" }
    }
  }
}
```
